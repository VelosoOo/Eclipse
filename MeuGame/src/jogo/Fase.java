...
public class Fase extends JPanel implements ActionListener {
	...
	private Image fundo;
	private Nave nave;
	private Timer timer; // atualização da tela
	...
	public Fase() {
		...
		timer = new Timer(5, this);// a cada milisegundos chamar a classe
		timer.start();
	...
	public void gerarInimigos(){
		...
		coord = new int [10][2];
		Random r = new Random();
		for(int i = 0 ; i < coord[i].length; i++){
			coord[i][0] = 456 + r.nextInt(260);
			coord[i][1] = 29 + r.nextInt(400);
			for(int j = 0; j < coord[j].length ; j++){
	...
	@Override
	public void actionPerformed(ActionEvent arg0) {// ou e//mecher a nave
